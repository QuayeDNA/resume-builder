...
function BulletRow({ bullet, expId, idx }: { bullet: string; expId: number; idx: number }) {
  ...
  return (
    <div className="flex gap-1.5 items-start">
      <textarea
        value={bullet}
        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => updateBullet(expId, idx, e.target.value)}
      ...
      <div className="flex flex-col gap-1 flex-shrink-0">
        <IconButton
          onClick={() =>
            run(key, () => aiImproveBullet(bullet, exp?.role || '', exp?.company || ''), (v) => updateBullet(expId, idx, v as string))
          }
...
function ExperienceEntry({ exp }: { exp: ExperienceEntry }) {
  ...
  return (
    <EntryCard onDelete={() => removeExperience(exp.id)}>
      <Input label="Company"    value={exp.company}  onChange={(v) => updateExperience(exp.id, 'company', v as string)}  placeholder="Acme Corp" />
      <Input label="Role/Title" value={exp.role}     onChange={(v) => updateExperience(exp.id, 'role', v as string)}     placeholder="Software Engineer" />
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <Input label="Start" value={exp.start} onChange={(v) => updateExperience(exp.id, 'start', v as string)} placeholder="Jan 2022" />
        <Input label="End"   value={exp.end}   onChange={(v) => updateExperience(exp.id, 'end', v as string)}   placeholder="Present" />
      </div>
        ...
        <Button
          onClick={() =>
            run(suggestKey, () => aiSuggestBullets(exp.role, exp.company), (bullets) =>
              appendBullets(exp.id, bullets as string[]),
            )
          }
          variant="ghost"
          size="sm"
