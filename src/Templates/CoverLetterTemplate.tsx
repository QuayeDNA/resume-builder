...
export default function CoverLetterTemplate({ resume, cl }: { resume: ResumeData; cl: CoverLetterData }) {
  ...
  return (
    ...
    >
      <div style={{ borderBottom: `3px solid ${t.accent}`, paddingBottom: '12px', marginBottom: '28px' }}>
        <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '22px', fontWeight: '700', margin: '0 0 6px 0', color: t.accent }}>
          {p.name || 'Your Name'}
        </h1>
      ...
      <div style={{ whiteSpace: 'pre-wrap', fontSize: '10.5px', color: '#333333', lineHeight: '1.85', marginBottom: '32px' }}>
        {cl.body || `I am writing to express my strong interest in the ${cl.role || '[Role]'} position at ${cl.company || '[Company]'}.\n\n[Use the AI Generate button in the Cover Letter tab to create a personalized letter, or write your own here.]`}
      </div>

      <div style={{ fontSize: '10.5px', fontWeight: '600', marginBottom: '36px' }}>Sincerely,</div>
      <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '16px', fontWeight: '700', color: t.accent }}>
        {p.name || 'Your Name'}
      </div>
    </div>
  )
